...
void ST7920::gotoXY(int x, int y)
{
    ...
    if(y >= 64 && y < 96)  { y -= 32; x += 0; } else
    if(y >= 96 && y < 128) { y -= 64; x += 8; }
...
void ST7920::fillScreen(unsigned char background)
{
  for(unsigned int j = 0; j < 32; j++) {
    ...
    for (unsigned int i = 0; i < 16; i++) {
        spi.write(background & 0xF0);   spi.write(background << 4);
    }
...
void ST7920::fillRectangle(unsigned char background, int x, int y, int width, int height)
{
...
void ST7920::drawMultipleVerticalLines(unsigned char times, int x, int y, int height)
{
    ...
    for (unsigned int j = 0; j < height; j++) {
        ...
        for (unsigned char i = 0; i < times; i++) {
            ...
            if ((i+1) % 4 == 0 || i == times - 1) {
                spi.write(line & 0xF0);   spi.write(line << 4);
                line = 0;
            }
        }
        spi.setCS(0);
...
void ST7920::printChar(const char c, int x, int y)
{
    char index;
    if (c == ' ') {
        index = ('z' - '0' + 1);
...
void ST7920::blinkString(const char* string, int x, int y, unsigned char times)
{
    for (unsigned char i = 0; i < times; i++) {
        if (i % 2 == 0) {
            printString(string, x, y);
        } else {
            fillRectangle(0x00, x * 2, y, strlen(string), 8);
        }
